...
namespace Presentacion.Administrador.Vistas
{
    internal class VistaInventario : UserControl, IVistaAdministrador
    {
        ...
        public VistaInventario()
        {
            ...
            var barraSuperior = new Panel { Dock = DockStyle.Top, Height = 48, Padding = new Padding(0, 12, 0, 0) };
            var btnNuevo = EstiloBoton.Primario(new Button { Text = "+  Nuevo producto", Size = new Size(180, 38), Dock = DockStyle.Right });
        ...
        private void CargarDatos()
        {
            ...
            {
                string estado = producto.Agotado ? "Agotado" : producto.StockBajo ? "Stock bajo" : (producto.Activo ? "Activo" : "Inactivo");
                ...
                Color colorEstado = producto.Agotado ? Paleta.Peligro : producto.StockBajo ? Color.FromArgb(217, 119, 6) : Paleta.Exito;
        ...
        {
            if (e.RowIndex < 0 || e.RowIndex >= _productos.Count) return;

            Producto producto = _productos[e.RowIndex];
            ...
            else if (columna == ColEliminar)
            {
                DialogResult confirmacion = MessageBox.Show(
                    $"¿Confirma que desea eliminar el producto \"{producto.Nombre}\"?",
